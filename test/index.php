...
foreach ($test_cases as $t => $v) {
	...
	foreach ($v as $key => $value) {
		if(gettype($v[$key])!="array"){
			echo "<b>".$key.":</b>".gettype($v[$key])."<br>";
		}else{
			echo "<b>".$key.":</b> array [<pre style='padding: 10px; margin: 0px;'>";

			foreach($v[$key][0] as $k => $c){
				echo "<b>".$k.":</b>".gettype($v[$key][0][$k])."<br>";
			}
			echo "</pre>]";
		}
	}
